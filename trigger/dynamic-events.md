...
| `player_join` | 玩家加入 | — |
| `player_quit` | 玩家退出 | — |
| `player_death` | 玩家死亡 | `death_message` |
| `player_respawn` | 玩家重生 | — |
| `player_interact` | 玩家交互 | `item` |
...
| `player_toggle_sneak` | 潜行切换 | `is_sneaking` |
| `player_toggle_sprint` | 疾跑切换 | `is_sprinting` |
| `player_item_consume` | 消耗物品 | `item` |
| `player_item_held` | 切换手持物品 | `previous_slot`, `new_slot` |
| `player_animation` | 玩家动画 | — |
| `player_bed_enter` | 上床 | — |
| `player_bed_leave` | 离床 | — |
| `player_bucket_fill` | 装满桶 | `bucket` |
| `player_bucket_empty` | 倒空桶 | `bucket` |
| `player_changed_world` | 切换世界 | `from`, `to` |
| `player_portal` | 进入传送门 | — |
| `player_teleport` | 传送 | `from`, `to`, `cause` |
...
| `player_item_mend` | 经验修补 | `item`, `experience` |
...
| 事件名 | 说明 | 上下文变量 |
|--------|------|-----------|
| `damage_entity` | 实体受伤 | `damage`, `entity_type`, `cause` |
| `kill_entity` | 实体被击杀 | `entity_type`, `killer` |
| `entity_shoot_bow` | 射箭 | `bow` |
